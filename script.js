    ...
    <div class="card-header">${icon}
      <div class="card-title">
        <div class="card-title-row">
          <h2>${m.name}</h2>
          <button class="card-fav-btn${isFav(m.url)?' active':''}" onclick="toggleFav('${m.url}');this.classList.toggle('active');this.textContent=isFav('${m.url}')?'⭐':'☆'" title="お気に入り">${isFav(m.url)?'⭐':'☆'}</button>
        </div>
        <div class="card-tags">
...
function renderFavorites(){
  ...
  if(!favUrls.length){
    wrap.innerHTML='<div class="fav-empty">お気に入りはまだありません。<br>モンスターカードの ☆ からお気に入り登録できます。</div>';
    return;
  }
  const favMonsters=favUrls.map(url=>monsters.find(m=>m.url===url)).filter(Boolean);
  wrap.innerHTML=`<div class="fav-list">${favMonsters.map(m=>`
    <div class="fav-item" onclick="favOpenMonster('${m.url}')">
      ...
      <div class="f-info">
        ...
        <div class="f-meta">
          ...
          <span>${m.family||''}系</span>
          <span>位階${m.order||'?'}</span>
        </div>
      </div>
      <button class="f-remove" onclick="event.stopPropagation();toggleFav('${m.url}');renderFavorites()" title="解除">削除</button>
    </div>`).join('')}
  </div>`;
}
